...
5. 以下の 4 つの観点で問題を特定する:
   ...
   **a. タグの重複・類似チェック**:
   ...
   - `work/`, `tech/`, `private/`, `books/` 配下のファイル名（拡張子を除く）を走査し、既存タグ一覧を構築する
   ...
   **e. tech セクションへの抽出提案**:
   - `## work`、`## private`、`## book` セクションの各エントリを走査し、技術的な学び・発見・調査を含む内容を検出する
...
1. `work/`, `tech/`, `private/`, `books/` 配下のファイル名から `.md` を除去してタグ一覧を得る
